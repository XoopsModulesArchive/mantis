...
class UserPreferences
{
    ...
    public $advanced_report;

    public $advanced_view;

    public $advanced_update;
    ...
    public $email_on_assigned;

    public $email_on_feedback;

    public $email_on_resolved;

    public $email_on_closed;

    public $email_on_reopened;

    public $email_on_bugnote;

    public $email_on_status;

    public $email_on_priority;
...
function user_pref_insert($p_user_id, $p_project_id, $p_prefs)
{
    ...
    foreach ($t_vars as $var => $val) {
        $t_values[] = '\'' . db_prepare_string($p_prefs->$var) . '\'';
    }
...
function user_pref_update($p_user_id, $p_project_id, $p_prefs)
{
    ...
    foreach ($t_vars as $var => $val) {
        $t_pairs[] = "$var = '" . db_prepare_string($p_prefs->$var) . '\'';
    }
...
function user_pref_get_pref($p_user_id, $p_pref_name, $p_project_id = ALL_PROJECTS)
{
    ...
    if (array_key_exists($p_pref_name, $t_vars)) {
