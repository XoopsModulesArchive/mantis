...
function graph_group($p_title = '')
{
    ...
    $gbplot = new GroupBarPlot([$p1, $p2, $p3]);
...
function create_bug_enum_summary_pct($p_enum_string, $p_enum)
{
    ...
    for ($i = 0; $i < $enum_count; $i++) {
        ...
        if ($total > 0) {
            ...
            $enum_name_count[] = db_result($result, 0) * 0;
        }
    } ### end for
}
...
{
    global $g_mantis_bug_table, $g_mantis_user_table, $g_mantis_project_category_table, $category_name, $category_bug_count;
...
{
    global $g_mantis_bug_table, $g_mantis_user_table, $g_mantis_project_user_list_table, $developer_name, $open_bug_count, $resolved_bug_count, $total_bug_count;
...
{
    global $g_mantis_bug_table, $g_mantis_user_table, $reporter_name, $reporter_count;
...
{
    global $g_mantis_bug_table, $g_mantis_project_category_table, $category_name, $category_bug_count;
...
function find_date_in_metrics($aDate)
{
    ...
    for ($i = 0, $iMax = count($metrics); $i < $iMax; $i++) {
