...
function gpc_get($p_var_name, $p_default = null)
{
    ...
    } elseif (func_num_args() > 1) { #check for a default passed in (allowing null)
        $t_result = $p_default;
    } else {
...
function gpc_get_string_array($p_var_name, $p_default = null)
{
    ...
    if (!((func_num_args() > 1 && $t_result === $p_default)
          || is_array($t_result))) {
...
function gpc_get_int_array($p_var_name, $p_default = null)
{
    ...
    for ($i = 0, $iMax = count($t_result); $i < $iMax; $i++) {
...
function gpc_string_to_bool($p_string)
{
    if (0 == strcasecmp('off', $p_string)
        || 0 == strcasecmp('no', $p_string) || 0 == strcasecmp('false', $p_string)
