...
function user_create($p_username, $p_password, $p_email = '', $p_access_level = null, $p_protected = false, $p_enabled = true)
{
    ...
    $g_cache_userid[$p_username] = $t_user_id; // RMV
...
# RMV: cache this mapping (get id from username)
$g_cache_userid = [];
...
{
    global $g_cache_userid;

    if (!empty($g_cache_userid[$p_username])) {
        return $g_cache_userid[$p_username];
    }
...
function user_get_email($p_user_id)
{
    ...
    if (is_blank($t_email) && module_has_integrated_users()) {
        $username = user_get_name($p_user_id);

        $email = module_user_email($username);
...
function user_set_name($p_user_id, $p_username)
{
    ...
    $g_cache_userid[$p_username] = $p_user_id;
