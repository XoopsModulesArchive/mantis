...
{
    global $g_complete_date_format, $g_show_view, $g_bugnote_order, $g_path, $g_email_separator1, $g_email_padding_length;
    ...
    if (0 != $vHandler_id) {
        $tHandler_name = user_get_name($vHandler_id);
    } else {
        $tHandler_name = '';
    }
    ...
    $t_message .= str_pad(lang_get('emailHandler') . ': ', $g_email_padding_length, ' ', STR_PAD_RIGHT) . $tHandler_name . "\n";
...
{
    global $g_mantis_bugnote_table, $g_mantis_bugnote_text_table, $g_complete_date_format, $g_bugnote_order, $g_email_separator2;
    ...
    for ($i = 0; $i < $bugnote_count; $i++) {
        ...
        $t_message .= $g_email_separator2 . "\n";

        $t_message .= $t_string . "\n";

        $t_message .= $g_email_separator2 . "\n";

        $t_message .= wordwrap($t_note) . "\n\n";
    }
...
function email_build_history_message($p_bug_id)
{
    ...
    for ($i = 0, $iMax = count($history); $i < $iMax; $i++) {
        $t_message .= str_pad($history[$i]['date'], 20) . str_pad($history[$i]['username'], 20) . str_pad($history[$i]['note'], 20) . str_pad($history[$i]['change'], 20) . "\n";
    }
...
{
    global $g_from_email, $g_enable_email_notification, $g_return_path_email, $g_use_x_priority, $g_use_phpMailer, $g_phpMailer_method, $g_smtp_host, $g_smtp_username, $g_smtp_password, $g_mail_priority;
    ...
    if (ON == $g_use_phpMailer) {
        ...
        $t_recipient_list = preg_split(',', $t_recipient);
        ...
        $t_bcc_list = preg_split(',', $p_header);
        ...
        if (true != $result) {
            ...
            print htmlspecialchars($t_recipient, ENT_QUOTES | ENT_HTML5) . '<br>';

            print htmlspecialchars($t_subject, ENT_QUOTES | ENT_HTML5) . '<br>';

            print nl2br(htmlspecialchars($t_headers, ENT_QUOTES | ENT_HTML5)) . '<br>';

            print nl2br(htmlspecialchars($t_message, ENT_QUOTES | ENT_HTML5)) . '<br>';
...
function email_bug_reminder($p_recipients, $p_bug_id, $p_message)
{
    ...
    $t_sender = current_user_get_field('username') . ' <' . current_user_get_field('email') . '>';
    ...
    foreach ($p_recipients as $t_recipient) {
        ...
        $t_contents = $t_header . string_get_bug_view_url($p_bug_id, $t_recipient) . "\n\n$p_message";
