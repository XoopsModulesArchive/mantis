...
function custom_field_update($p_field_id, $p_def_array)
{
    ...
    $query = 'UPDATE ' . config_get('mantis_custom_field_table') . ' SET ';
...
function custom_field_set_value($p_field_id, $p_bug_id, $p_value)
{
    ...
    if (!is_blank($t_valid_regexp)) {
        if (!preg_match($t_valid_regexp, $p_value)) {
            return false;
        }
    }

    if (mb_strlen($p_value) < $t_length_min) {
        return false;
    }

    if ((0 != $t_length_max) && (mb_strlen($p_value) > $t_length_max)) {
        return false;
    }
