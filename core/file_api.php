...
function file_list_attachments($p_bug_id)
{
    ...
    $t_can_delete = ($t_bug->status < config_get('bug_resolved_status_threshold')) && access_has_project_level(config_get('handle_bug_threshold'));
    ...
    for ($i = 0; $i < $num_files; $i++) {
        ...
        echo '</a>&nbsp;' . $t_href . file_get_display_name($v_filename) . "</a> ($t_filesize bytes) <span class=\"italic\">$t_date_added</span>";
        ...
        if (($v_filesize <= config_get('preview_attachments_inline_max_size'))
            && (0 != $v_filesize)
            && (in_array(mb_strtolower(file_get_extension($v_diskfile)), ['png', 'jpg', 'gif', 'bmp'], true))) {
...
function file_delete_project_files($p_project_id)
{
    ...
    if ((DISK == $t_method) || (FTP == $t_method)) {
        ...
        $query = "SELECT diskfile, filename FROM $t_project_file_table" . " WHERE project_id=$p_project_id";
...
function file_add($p_bug_id, $p_tmp_file, $p_file_name, $p_file_type = '')
{
    ...
    } elseif (is_uploaded_file($p_tmp_file)) {
