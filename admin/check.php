...
{
    if ((0 === $p_result) || ('no' === mb_strtolower($p_result))) {
        echo 'No';
    }

    if ((1 === $p_result) || ('yes' === mb_strtolower($p_result))) {
        echo 'Yes';
    }
}
...
if (module_has_integrated_display()) {
    module_display_admin_header('', 'Mantis Administration - Check Installation');
} else {
    ?>
<html>
<head>
...
<table width="100%" bgcolor="#222222" border="0" cellpadding="10" cellspacing="1">
    ...
    <tr>
        <td bgcolor="#ffffff">
            <?php if (module_has_integrated_database()) { ?>
                Checking connection to <?php echo module_type(); ?> database
            <?php } else { ?>
    ...
    <tr>
        ...
        if (('\\' == mb_substr($t_absolute_path, -1, 1))
            || ('/' == mb_substr($t_absolute_path, -1, 1))) {
            print_test_result(GOOD);
        } else {
            print_test_result(BAD);
        }
    ...
    <?php
    # Windows-only checks
    if ('Windows' == mb_substr(php_uname(), 0, 7)) {
        ?>
        <!-- Email Validation -->
        <tr>
            <td bgcolor="#ffffff">
    ...
    <table width="100%" bgcolor="#222222" border="0" cellpadding="20" cellspacing="1">
        <tr>
            <td bgcolor="#ffcc22">
                ...
                You have register_globals enabled in PHP, which is considered a security risk. Since version 0.18, Mantis has no longer relied on register_globals being enabled. PHP versions later that 4.2.0 have this option disabled by default. For more information on the security issues associated
                with enabling register_globals, see <a href="http://www.php.net/manual/en/security.registerglobals.php">this page</a>.

                If you have no other PHP applications that rely on register_globals, you should add the line
                ...
                <pre>php_value register_globals off</pre>
                to a <tt>.htaccess</tt> file or a <tt>&lt;Directory&gt;</tt> or <tt>&lt;Location&gt;</tt> block in your apache configuration file. See the apache documentation if you require more information.
            </td>
        </tr>
    </table>
...
if (CRYPT_FULL_SALT == config_get('login_method')) { ?>
    ...
    <br><?php
} elseif (MD5 != config_get('login_method')) { ?>
    <br>
...
<table width="100%" bgcolor="#222222" border="0" cellpadding="20" cellspacing="1">
    <tr>
        <td bgcolor="#f4f4f4">
            ...
            <p>You can test the mail() function with this form. Just check the recipient and click submit. If the page takes a very long time to reappear or results in an error then you will need to investigate your php/mail server settings. Note that errors can also appear in the server error log.
                More help can be found at the <a href="http://www.php.net/manual/en/ref.mail.php">PHP website</a>.</p>
            <?php
            if ($f_mail_test) {
