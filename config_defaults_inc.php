...
$g_use_iis = (false !== mb_strstr($_SERVER['SERVER_SOFTWARE'], 'IIS')) ? ON : OFF;
...
# Choose default from CMS, if available
if (module_is_module()) {
    $language = module_current_user_language();

    if (!$language) {
        $language = module_config_default_language();
    }

    if (false !== $language && in_array($language, $_language_choices_arr, true)) {
...
if (module_has_integrated_users()) {
    # Force use of CMS permissions for anonymous access

    $g_allow_anonymous_login = ON;

    # NOTE: $g_anonymous_account takes priority for
    # mantis name for anonymous user.  If left blank, then
    # CMS name applies.
}
...
if (module_has_integrated_database()) {
    $g_db_table_prefix = module_db_prefix($g_db_table_prefix);
}
...
#  outputHandler=ob_gzhandler) this option will be ignored.
#
...
if (module_has_integrated_display()) {
    # FIXME: This is not module-type independent...

    $g_css_include_file = $g_path . 'css/xoops_default.css';
} else {
...
if (module_has_integrated_users()) {
    $g_logout_redirect_page = $g_path . 'main_page.php';
} else {
